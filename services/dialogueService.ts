...
export const NPC_PERSONAS: Record<string, NPCPersona> = {
  ...
  mum_kitchen: {
    ...
    knowledge: [
      ...
      'Dessert recipes: Crepes with Strawberry jam, Marzipan chocolates, Vanilla ice cream',
      'Baking recipes: Bread, Cookies, Chocolate cake',
      "Tea is made at the kitchen fireplace, where the kettle hangs — from the recipe book or with Make Tea. You supply the ingredients for your child's first practice cup; after that a cup needs tea leaves, water and milk from the shop",
      'Cooking only works beside a fire: your kitchen fireplace, or a campfire your child buys at the general store and sets down outdoors. The recipe book can be read anywhere',
...
function getStaticDialogue(npc: NPC, currentNodeId: string): DialogueNode | null {
  ...
  const availableNodes = allDialogueNodes.filter((node) => {
    ...
    if (node.requiredQuest) {
      ...
      if (node.maxQuestStage !== undefined) {
        const questStage = gameState.getQuestStage(node.requiredQuest);
        if (questStage > node.maxQuestStage) return false;
      }
    }
    ...
    if (node.hiddenIfQuestStageDaysElapsed) {
      const { questId, days } = node.hiddenIfQuestStageDaysElapsed;
      const daysSince = eventChainManager.getDaysSinceStageEntered(questId);
      if (daysSince !== null && daysSince > days) return false;
    }
  ...
  const filteredResponses = node.responses?.filter((response) => {
    ...
    if (response.hiddenIfQuestStageDaysElapsed) {
      const { questId, days } = response.hiddenIfQuestStageDaysElapsed;
      const daysSince = eventChainManager.getDaysSinceStageEntered(questId);
      if (daysSince !== null && daysSince > days) return false;
    }
