...
export interface CropDefinition {
  ...
  // Water requirements (real ms, derived from game days — see WATER_NEEDED etc.)
  waterNeededInterval: number; // How long after watering before the plant needs water again
  wiltingGracePeriod: number; // How long past that before the plant starts wilting
  deathGracePeriod: number; // How long after wilting starts before the plant dies
...
const WATER_NEEDED = 1 * GAME_DAY; // needs watering every game day (2 real hours)
const WILTING_GRACE = 2 * GAME_DAY; // droops for 2 game days before it is beyond saving
const DEATH_GRACE = 1 * GAME_DAY; // dies 1 game day after wilting starts
