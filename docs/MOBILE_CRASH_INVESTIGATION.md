...
- An uninterrupted two-minute check against the production build in local Chromium stayed in the village with 175 textures / 299 MiB, 896 scene nodes and a 1688 × 780 framebuffer throughout. Reported JS heap varied roughly 22–34 MiB rather than growing steadily, and no page errors or reloads occurred. This does not rule out a physical iPhone process-budget or driver failure. A prior development-server run was discarded because source edits triggered hot reload during measurement.
