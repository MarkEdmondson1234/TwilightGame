...
## Grounding and framing fix (approved follow-up)

- Mobile pilot players use visible alpha footprints from `data/playerSpriteFootprints.json`. The hand-drawn sprites have transparent padding, so the image rectangle bottom is not their feet. Local and remote players use the same footprint offset; local speech/emote positions follow the adjusted sprite centre. Desktop retains centre anchoring.
- Footprints are measured offline by `node scripts/generate-player-footprints.mjs`; no artwork is rewritten and no mobile GPU readback or additional textures are needed. Regenerate after changing player images. Tests compare representative metadata to the actual source alpha.
- The room has a reserved lower controls strip, up to 88 CSS pixels. Coverage, artwork, camera pan and inverse taps use its reduced viewport together. The camera follows the visible body centre; maximum zoom accounts for visible character height, with unavailable presets disabled. The visible-height envelope spans walk frames to avoid camera bobbing from alpha-bound changes.
