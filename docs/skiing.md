...
- The opening field gives warning time, and spawns preserve a free lane in each nearby depth band. Speed/density increase with depth; wolves appear from depth 4. Poor, medium and fine logs appear from depths 1, 3 and 5.
...
Score is distance / 10 (rounded down), plus 50/100/150 per poor/medium/fine log collected. Crashes still count as runs; salvage affects inventory, not the score. Personal records are saved locally per starting depth. Version 2 global boards also separate starting depths, so a deep-forest start is not compared with an entrance start.

`firebase/skiingScoreService.ts` stores one best per signed-in account at `skiingBoards/v2_<starting depth>/scores/<uid>`. A transaction never replaces a higher score with a lower one. The UI shows global best and the top five skiers. Network failure does not prevent play, and the saved local best is retried when the board is available on a later visit. Records display the existing account display name (or “Forest skier”). These are friendly, client-reported records, not an anti-cheat competitive system.
