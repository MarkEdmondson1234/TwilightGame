...
const NPCInteractionIndicators: React.FC<NPCInteractionIndicatorsProps> = ({
    ...
    return (
        <>
            {npcs.filter(npc => NPCS_WITH_INDICATORS.includes(npc.id)).map((npc) => {
                ...
                const indicatorSize = TILE_SIZE * Math.min(npcScale / 2, 2);
                ...
                return (
                    <React.Fragment key={`npc-indicator-${npc.id}`}>
                        ...
                        <div
                            ...
                            style={{
                                left: npc.position.x * TILE_SIZE + offsetX - indicatorSize / 4 + TILE_SIZE,
                                top: npc.position.y * TILE_SIZE + offsetY - indicatorSize / 2,
                                width: indicatorSize,
                        ...
                        {isInRange && (
                            <div
                                ...
                                style={{
                                    left: npc.position.x * TILE_SIZE + offsetX + TILE_SIZE / 2 + TILE_SIZE,
                                    top: npc.position.y * TILE_SIZE + offsetY - indicatorSize / 2 - 20,
