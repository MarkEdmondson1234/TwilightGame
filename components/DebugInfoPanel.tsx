...
const DebugInfoPanel: React.FC<DebugInfoPanelProps> = ({
  ...
  const clickedColorTrace = clickedTileData
    ? ColorResolver.traceTileColor(clickedTileData.type)
    : null;
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 bg-black/70 p-3 rounded-lg border border-slate-700 text-sm text-white font-mono space-y-1 ${zClass(Z_DEBUG_PANEL)}`}
    >
      <h3 className="text-md font-bold text-cyan-300 mb-2">Debug Info (F3)</h3>
      <div>
        Player Pos: ({playerPos.x.toFixed(2)}, {playerPos.y.toFixed(2)})
      </div>
      <div>
        Current Tile: ({currentTile.x}, {currentTile.y})
      </div>
      {tileData && (
        <>
          ...
          <div>
            Is Solid:{' '}
            <span
              className={isTileSolid(tileData.collisionType) ? 'text-red-500' : 'text-green-500'}
            >
              {isTileSolid(tileData.collisionType).toString()}
            </span>
          </div>
        </>
      )}
      <div className="pt-1 mt-1 border-t border-slate-600">
        ...
        <div>
          X Tiles: [{minTileX}, {maxTileX}]
        </div>
        <div>
          Y Tiles: [{minTileY}, {maxTileY}]
        </div>
      </div>
      <div className="pt-1 mt-1 border-t border-slate-600">
        ...
        <div>
          Season: <span className="text-yellow-300">{gameTime.season}</span> Day {gameTime.day}
        </div>
        <div>
          Time:{' '}
          <span className={gameTime.timeOfDay === 'Day' ? 'text-yellow-200' : 'text-blue-300'}>
            {gameTime.timeOfDay}
          </span>{' '}
          (Hour {gameTime.hour})
        </div>
      ...
      {clickedTile && clickedTileData && (
        <div className="pt-1 mt-1 border-t border-slate-600">
          ...
          <div>
            Type: <span className="text-gray-400">{clickedTileData.type}</span>
          </div>
          <div>
            Solid:{' '}
            <span
              className={
                isTileSolid(clickedTileData.collisionType) ? 'text-red-500' : 'text-green-500'
              }
            >
              {isTileSolid(clickedTileData.collisionType).toString()}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span>Colour:</span>
