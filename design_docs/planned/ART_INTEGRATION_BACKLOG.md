...
- **`incoming-art/` housekeeping.** The staging folder is gitignored (added after the dress
  sprint) — originals live only on disk, and `git add -A` cannot pick them up by accident.
  Integrated pieces have tracked copies under `public/assets/`, so the folder is safe to
  empty once the backlog is triaged.
- **Optimised UI icons with no references** (`public/assets-optimized/icons/ui/d-pad *.png`,
  `mushroom*.png` — spaced-name variants) are tracked but referenced by nothing: TouchControls
  uses `/assets/ui/dpad_*.png` and `iconAssets.ts` registers only the action icons plus
  `lock`/`door`/`garden`. They were committed with the d-pad PR; delete them (and their sources)
  if the spaced-name versions are truly surplus.
