...
out to be bugs in the _tests_, not the game data, and are fixed:
...
| `proceduralDeterminism.test.ts` | `Math.random()` or a clock read creeping back into `maps/procedural.ts`, so two players holding the same `forest_<seed>` id rebuild different worlds | Touching any procedural generator |
| `proceduralDepth.test.ts` | A forest/cave/lava depth counter that fails to reset on the way out — depth is half the map seed, so a drifted counter puts two players in different worlds | Adding a transition out of a procedural chain |
| `sharedMaps.test.ts` | The shared-world systems disagreeing about which maps other players exist on, or a random shop becoming shared | Changing `MULTIPLAYER.SHARED_MAPS` or `multiplayer/sharedMaps.ts` |
| `sharedWorldAuthRetry.test.ts` | A per-map room controller that never retries when the player signs in — Firebase signs in after the first map loads, so it silently joins nothing for the whole session | Adding a controller that joins a shared room |
| `sharedWorldObservability.test.ts` | A shared-world step failing quietly: the presence ghost check judged on the device clock (a tablet minutes fast drops every live player), dropped presence records or clock skew going unreported, a wreath/painting picture that never reached the cloud or could not be fetched with no reason recorded; signing out leaving a ghost of you behind (the record can only be deleted while signed in); a listener the server cancelled going silent | Touching presence, `multiplayer/serverClock.ts`, painting storage or placed-item hydration |
| `sharedFarmAuthRetry.test.ts` | The shared garden listener starting before sign-in or before the Firebase module has loaded, and never retrying — one player's plots flush, the other's never arrive, until they walk out of the village and back | Touching `startSharedSync`, `communityGardenService.startListening`, or any manager (not hook) that subscribes to a shared collection |
| `localSaveOwner.test.ts` | Signing in on a shared laptop uploading the previous player's local game over this account's cloud save because it was "newer" — a garden you never planted, an inventory that is not yours | Touching `syncManager.onSignIn`, the local save timestamp, or `decideSignInSync()` |
| `sharedBattle.test.ts` | The shared-battle wire, spectator rules (never watch your own fight, drop a stale one), and client/server parity on the battle security rules | Touching combat sharing, cheering, or `database.rules.json` |
...
| `sessionDiagnostics.test.ts` | Bounded diagnostics growing a leak: logs sent while inactive/hidden, the log cap breaking, or slow-minute runtime context appearing on healthy minutes | Touching `utils/sessionDiagnostics.ts` |
| `persistenceReporting.test.ts` | A save/load failure or self-heal fallback that never reaches Sentry — silent data loss in production | Touching `CharacterData`, `CookingManager` self-heal, or the `reportErrorOnce` wiring |
| `mapSpawnReporting.test.ts` | A spawn fallback or validation failure that stops reporting — mis-authored transitions and trapped-player maps become invisible again | Touching `MapManager.transitionToMap`/`loadMap` reporting |
| `gifts.test.ts` | A gift opened by the wrong player (the transport hands every client every document, so recipient filtering is the codec's job), or a malformed record crashing mid-render instead of being ignored | Touching `multiplayer/gifts.ts` |
| `giftRules.test.ts` | The gift name cap or required-field shape in `firestore.rules` drifting from `multiplayer/gifts.ts` — the server-side rule is the one that holds, and a missing match block denies every send | Touching gift wire fields or `firestore.rules` |
...
| `cookingStations.test.ts`, `teaBook.test.tsx`, `teaInteraction.test.ts`, `cookingStationIndicators.test.tsx`, `campfireItem.test.ts` | Cooking working away from a fire again (#151/#157), the book's Cook button, the "Cook here" interaction/button and the station glow disagreeing about what "near a fire" means, or a placed campfire being carried off by the click meant to cook on it | Touching `utils/cookingStations.ts`, the recipe book, Mum's fireplace, or the campfire item |
