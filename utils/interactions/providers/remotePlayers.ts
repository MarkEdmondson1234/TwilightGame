...
export function remotePlayerProvider(ctx: InteractionContext): AvailableInteraction[] {
  const { position, isContextMenu, onEmote, onOpenEmoteWheel, onStartChat, onGiftPlayer } = ctx;
  ...
  let nearest: { uid: string; name: string; distance: number } | null = null;
  for (const player of remotePlayerManager.getRemotePlayers()) {
    ...
    if (!nearest || distance < nearest.distance) {
      nearest = { uid: player.uid, name: player.name, distance };
    }
  }
  if (!nearest) return [];

  const interactions: AvailableInteraction[] = [];
  const { uid, name } = nearest;
  ...
  // The gesture NPCs have: pick something from your bag, they get it. Only
  // offered when a gift path exists — like the emote options, the option is
  // inert without its callback.
  if (onGiftPlayer) {
    interactions.push({
      type: 'player_gift',
      label: `Give a Gift`,
      icon: '🎁',
      color: '#ec4899',
      data: { playerUid: uid, playerName: name },
      execute: () => onGiftPlayer(uid, name),
    });
  }
