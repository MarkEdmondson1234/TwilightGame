...
> = {
  // Large crops (2.5x2.5 tiles) - spill over neighbouring tiles for a lush, abundant look
  tomato: { width: 2.5, height: 2.5, offsetX: -0.75, offsetY: -1.5 },
  ...
  melon: { width: 2, height: 2, offsetX: -0.5, offsetY: -1 },
  broccoli: { width: 2, height: 2, offsetX: -0.5, offsetY: -1 },
  ...
  private renderTile(
    ...
    if (!sprite || sprite instanceof PIXI.Graphics) {
      ...
      if (cropConfig) {
        ...
        // Depth-sort crops by Y position (like player/NPCs)
        // Seedlings stay flat; taller crops use soil bottom as "feet"
        sprite.zIndex =
          growthStage === CropGrowthStage.SEEDLING
            ? Z_TILE_SPRITES
